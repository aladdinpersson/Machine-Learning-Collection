...
class Train_MNIST(object):
    def  __init__(self):
        ...
        if self.args.create_validationset:
            self.loader_train, self.loader_validation, self.loader_test = self.prepare_data()
    ...
    def prepare_args(self):
        ...
        parser.add_argument('--weight-decay', default = 1e-5, type=float,
                            metavar='R', help='L2 regularization lambda')
        parser.add_argument('--momentum', default = 0.9, type=float,
                            metavar='M', help='SGD with momentum')
        parser.add_argument('--epochs', type=int, default=100, metavar='N',
                            help='number of epochs to train (default: 100)')
        parser.add_argument('--batch-size', type=int, default=128, metavar='N',
                            help='input batch size for training (default: 128)')
        parser.add_argument('--log-interval', type=int, default=240, metavar='N',
                           help='how many batches to wait before logging training status')
        parser.add_argument('--seed', type=int, default=1, metavar='S',
                            help='random seed (default: 1)')
        parser.add_argument('--number-workers', type=int, default=0, metavar='S',
                            help='number of workers (default: 0)')
        parser.add_argument('--init-padding', type=int, default=2, metavar='S',
                            help=' If use initial padding or not. (default: 2 because mnist 28x28 to make 32x32)')
        parser.add_argument('--create-validationset', action='store_true', default=False,
    ...
    def prepare_data(self, shuffle=True):
        ...
        data = self.dataset(shuffle, self.transform_train, self.transform_test, self.args.number_workers, self.args.create_validationset, self.args.batch_size,
                                        validation_size = 0.1, random_seed = self.args.seed)
    ...
    def train(self):
        ...
        loss_list, batch_list, epoch_list, validation_acc_list, training_acc_list =  [], [], [0], [0], [0]
            ...
            for batch_idx, (x, y) in enumerate(self.loader_train):
                ...
                if batch_idx % self.args.log_interval == 0:
                    ...
                    if train_acc > self.best_acc and self.args.save_model:
                        ...
                        save_checkpoint(self.filename, self.model, self.optimizer, self.best_acc, epoch)
    ...
    def choose_network(self):
        self.model = LeNet(in_channels = self.in_channels, init_weights = True, num_classes = 10)
    ...
    def main(self):
        if __name__ == '__main__':
            ...
            if self.args.resume:
                self.model.eval()
                self.model,  self.optimizer, self.checkpoint, self.start_epoch, self.best_acc = load_model(self.args, self.model, self.optimizer)
            else:
