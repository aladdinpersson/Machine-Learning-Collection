...
class ResNet(nn.Module):
    def __init__(self, residual_template, layers, image_channel, num_classes = 10):
        ...
        self.conv1 = nn.Conv2d(in_channels = image_channel, out_channels = 64, kernel_size = 3, stride = 1, padding = 1, bias = False)
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace = True)
        self.layer1 = self._make_layer(residual_template, layers[0], channels = 64, stride = 1)
        self.layer2 = self._make_layer(residual_template, layers[1], channels = 128,  stride = 2)
        self.layer3 = self._make_layer(residual_template, layers[2], channels = 256, stride = 2)
        self.layer4 = self._make_layer(residual_template, layers[3], channels = 512, stride = 2)
    ...
    def _make_layer(self, residual_template, num_residuals_blocks, channels, stride):
        ...
        if stride != 1 or self.in_channels != channels * residual_template.expansion:
            identity_downsample = nn.Sequential(
                nn.Conv2d(self.in_channels, channels * residual_template.expansion,
                                  kernel_size = 1, stride = stride, bias = False),
                nn.BatchNorm2d(channels * residual_template.expansion),
            )

        layers = []
        layers.append(residual_template(self.in_channels, channels, stride, identity_downsample))
