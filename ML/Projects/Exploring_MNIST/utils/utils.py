...
class visdom_plotting(object):
    ...
    def create_plot(self, loss_list, batch_list, validation_acc_list, epoch_list, training_acc_list):

        if self.viz.check_connection():
             self.cur_batch_win = self.viz.line(torch.FloatTensor(loss_list), torch.FloatTensor(batch_list),
             ...
             self.cur_validation_acc = self.viz.line(torch.FloatTensor(validation_acc_list), torch.FloatTensor(epoch_list),
                                                      win=self.cur_validation_acc, name='current_validation_accuracy',
                                                      update=(None if self.cur_validation_acc is None else 'replace'),
                                                      opts=self.cur_validation_acc_opts)

             self.cur_training_acc = self.viz.line(torch.FloatTensor(training_acc_list), torch.FloatTensor(epoch_list),
                                                      win=self.cur_validation_acc, name='current_training_accuracy',
